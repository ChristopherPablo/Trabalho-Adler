			...
			public void onClick(View v)
			{
				...
				if(login.getText().length() == 0 || senha.getText().length() == 0)
				{
					...
					if(opSpinner == "Usuário")
					{
						...
						{
							Toast sucess = Toast.makeText(MainActivity.this, "Você entrou no sistema como USUÁRIO!", Toast.LENGTH_SHORT);
							sucess.show();
						}
						else
						{
					...
					}else if(opSpinner == "Administrador")
					{
						...
						if(senhastr.equals(DBpassword))
						{
							Toast sucess = Toast.makeText(MainActivity.this, "Você entrou no sistema como ADMINISTRADOR!", Toast.LENGTH_SHORT);
							sucess.show();
						}
						else
						{
